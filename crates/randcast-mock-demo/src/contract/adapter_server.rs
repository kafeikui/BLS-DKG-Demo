...
use randcast_mock_demo::contract::{
    adapter::{
        Adapter, AdapterMockHelper, AdapterTransactions as ModelAdapterTrxs,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let adapter_rpc_endpoint = match args.next() {
        Some(arg) => arg,
        None => panic!("Didn't get a adapter rpc endpoint string"),
    };

    let addr = adapter_rpc_endpoint.parse()?;
    let initial_entropy = 0x8762_4875_6548_3456;
    ...
    let adapter = Adapter::new(initial_entropy, adapter_rpc_endpoint);

    let arc = Arc::new(RwLock::new(adapter));

    Server::builder()
        .add_service(AdapterTransactionsServer::with_interceptor(
            MockAdapter::new(arc.clone()),
            intercept,
        ))
